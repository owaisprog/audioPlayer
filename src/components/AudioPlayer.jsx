...
const AudioPlayer = () => {
  ...
  const { volume, changeVolume, toggleMute, isMute } = usePlayer();
  ...
  return (
    <div className="container my-4 mx-auto p-4">
      ...
      {isRecording ? (
        <Loader text={"Iniziamo! Carica un file audio"} />
      ) : (
        <>
          {/* Replace this condition with a loading state check */}
          {!loading ? (
            <div className="my-4 bg-[#a374b3] p-4 rounded-lg ">
              <div className="bg-[#d3c0d9] rounded-full p-4 text-[#58465f] flex flex-col sm:flex-row justify-between items-center">
                ...
                <div className="flex items-center space-x-4 mt-2 sm:mt-0">
                  <button
                    onMouseLeave={() => setIsVHover(false)}
                    disabled={!shifter}
                  >
                    {isMute ? (
                      <FaVolumeMute
          ...
          <div className="container mx-auto flex mt-6 flex-col md:flex-row gap-8 justify-center items-center">
            ...
            {!playing ? (
              <button
                id="playBtn"
                onClick={play}
                disabled={!shifter || recordedFile}
                className={`${
                  (!shifter || recordedFile) && " opacity-50"
              ...
              <button
                id="pausebtn"
                onClick={pause}
                className={`bg-no-repeat bg-contain w-40 h-16`}
                style={{
